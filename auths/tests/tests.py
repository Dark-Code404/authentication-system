...
def test_user_role(create_user):
    """
    Test that verifies the correct roles are assigned to the users.
    """
...
def test_user_login_code(client, create_user):
    ...
    Verifies the correct HTTP status code is returned on login.
    """
...
def test_user_login_response(client, create_user):
    """
    Test login for non-admin users, ensuring the appropriate message is returned.
    """
    res = client.post(reverse("login"), {"username": "yunish2", "password": "aaa123aaa"})
...
def test_parameter_login(client, create_user, username, password, result):
    """
    Parametrized test for login functionality, validating status code for various users.
    """
...
def test_parameter_register(client, create_user, username, password1, password2, role, result, msg):
    """
    Parametrized test for user registration, validating different username, password, and role scenarios.
    """
